...
contract WIZEToken is ERC20, Ownable {
    ...
    // sell fees
    uint256 public sellDividendRewardsFee = 13;
    uint256 public selltreasuryFee = 1;
    uint256 public selTeamFee = 1;
    ...
    function isExcludedFromFees(address account) public view returns (bool) {
      ...
      event DividendTokenUpdated(address indexed newAddress, address indexed oldAddress);
    ...
    function updateDividendTokenAddress(address newDividendToken) public onlyOwner {
        ...
        // Exclude new dividend token from dividends
        dividendTracker.excludeFromDividends(newDividendToken);
    ...
    function _transfer(
        ...
        if (tradingIsEnabled && automatedMarketMakerPairs[to]) {
            ...
            if (!swapping && canSwap) {
                ...
                swapAndLiquify(
                    contractTokenBalance.mul(buyLiquidityFee).div(buyTotalFees)
                );
                if (rewardEnable) {
        ...
        if (takeFee) {
            ...
            if (automatedMarketMakerPairs[to]) {
                fees = amount.div(100).mul(sellTotalFees);
                if (sellBurnFee > 0) {
                    burnAmount = amount.div(100).mul(sellBurnFee);
                }
            } else {
                fees = amount.div(100).mul(buyTotalFees);
                if (buyBurnFee > 0) {
                    burnAmount = amount.div(100).mul(buyBurnFee);
                }
            }
    ...
    function swapAndSendBnb(uint256 tokenAmount, uint256 totalFee) private {
        ...
        transferToWallet(
            payable(teamWallet),
            address(this).balance.div(totalFee).mul(buyTeamFee).div(2)
        );
        transferToWallet(
            payable(treasurywallet),
            address(this).balance.div(totalFee).mul(buytreasuryFee)
        );
        transferToWallet(
            payable(marketingWallet),
            address(this).balance.div(totalFee).mul(buyMarketingFee)
        );
    }
...
contract WIZETokenDividendTracker is DividendPayingToken, Ownable {
    ...
    function includeInDividends(address account, uint256 balance)
        external
        onlyOwner
    {
        require(excludedFromDividends[account]);
        ...
        _setBalance(payable(account), balance);
